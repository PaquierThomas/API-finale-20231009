...
app.get("/recipes/cuisine/:cuisineId", (req, res) => {
  ...
  db.all(
    "SELECT * FROM Recipes WHERE cuisine_id = ?",
    [cuisineId],
    (err, rows) => {
      if (err) {
...
app.get("/recipes/:recipeId", (req, res) => {
  ...
  db.get(
    ...
    (err, recipe) => {
      if (err) {
        console.error(
          "Error fetching recipe with basic information:",
          err.message
        );
        res.status(500).json({ error: "Internal server error" });
        return;
        ...
        `,
          ...
          `,
                ...
                (err, dietaryInfo) => {
                  if (err) {
                    console.error(
                      "Error fetching dietary information:",
                      err.message
                    );
                    res.status(500).json({ error: "Internal server error" });
                    return;
...
app.post("/recipes/:recipeId/add/ingredients", (req, res) => {
  ...
  function insertIngredientIntoRecipe() {
    ...
    db.run(
      "INSERT INTO RecipeIngredients (recipe_id, ingredient_id, quantity) VALUES (?, (SELECT ingredient_id FROM Ingredients WHERE name = ?), ?)",
      ...
      function (err) {
        ...
        db.all(
          "SELECT i.name, ri.quantity, i.unit FROM RecipeIngredients ri JOIN Ingredients i ON ri.ingredient_id = i.ingredient_id WHERE ri.recipe_id = ?",
          [recipeId],
          (err, ingredients) => {
            if (err) {
              console.error(
                "Error fetching ingredients for the recipe:",
                err.message
              );
              res.status(500).json({ error: "Internal server error" });
              return;
...
app.post("/recipes/add", (req, res) => {
  ...
  db.run(
    "INSERT INTO Recipes (recipe_name, recipe_description, image_url, cuisine_id) VALUES (?, ?, ?, ?)",
    [recipe_name, recipe_description, image_url, cuisine_id],
    function (err) {
      if (err) {
      ...
      db.get(
        "SELECT goal_id FROM Goals WHERE name = ?",
        [goal_name],
        (err, goal) => {
          if (err) {
      ...
      function insertRecipeData() {
        ...
        if (dietary_info_names && dietary_info_names.length > 0) {
          dietary_info_names.forEach((dietary_info_name) => {
            db.get(
              "SELECT diet_id FROM DietaryInformation WHERE name = ?",
              [dietary_info_name],
              (err, dietaryInfo) => {
                if (err) {
                  console.error(
                    "Error checking dietary information existence:",
                    err.message
                  );
                  res.status(500).json({ error: "Internal server error" });
                  return;
                ...
                if (!dietaryInfo) {
                  ...
                  db.run(
                    ...
                    function (err) {
                      if (err) {
                        console.error(
                          "Error creating dietary information:",
                          err.message
                        );
                        res
                          .status(500)
        ...
        function insertRecipe() {
          ...
          if (allergies_info_names && allergies_info_names.length > 0) {
            allergies_info_names.forEach((allergies_info_name) => {
              db.get(
                "SELECT allergy_id FROM AllergiesInformation WHERE name = ?",
                [allergies_info_name],
                (err, allergiesInfo) => {
                  if (err) {
                    console.error(
                      "Error checking allergies information existence:",
                      err.message
                    );
                    res.status(500).json({ error: "Internal server error" });
                    return;
                  ...
                  if (!allergiesInfo) {
                    ...
                    db.run(
                      ...
                      function (err) {
                        if (err) {
                          console.error(
                            "Error creating allergies information:",
                            err.message
                          );
                          res
                            .status(500)
                        ...
                        if (
                          allergiesInfoIds.length ===
                          allergies_info_names.length
                        ) {
                    ...
                    if (
                      allergiesInfoIds.length === allergies_info_names.length
                    ) {
                      // All allergies information has been processed
                      insertRecipeIngredients();
                    }
          ...
          function insertRecipeIngredients() {
            ...
            if (instructions && instructions.length > 0) {
              instructions.forEach(({ step_number, StepInstruction }) => {
                db.run(
                  "INSERT INTO InstructionStep (recipe_id, step_number, StepInstruction) VALUES (?, ?, ?)",
                  [recipeId, step_number, StepInstruction],
                  (err) => {
                    if (err) {
...
app.put("/recipes/:recipeId/update", (req, res) => {
  ...
  for (const attribute in updatedRecipe) {
    if (validAttributes.includes(attribute)) {
      ...
      if (
        updatedRecipe[attribute] !== undefined &&
...
app.put("/recipes/:recipeId/instructions/:stepNumber/update", (req, res) => {
  ...
  if (!updatedStep || !updatedStep.StepInstruction) {
    res.status(400).json({
      error:
        "Updated instruction step data is required, and StepInstruction must be provided",
    });
    return;
  }
...
app.delete(
  "/recipes/:recipeId/ingredients/:ingredientId/delete",
  (req, res) => {
    ...
    db.run(deleteQuery, deleteParams, function (err) {
      ...
      if (this.changes === 0) {
        ...
        res.json({
          message: "Ingredient deleted successfully from the recipe",
        });
      }
    });
  }
...
app.delete(
  "/recipes/:recipeId/instructions/:instructionId/delete",
  (req, res) => {
    const { recipeId, instructionId } = req.params;

    // Construct the SQL query to delete the instruction step from the recipe
    const deleteQuery =
      "DELETE FROM InstructionStep WHERE recipe_id = ? AND instruction_id = ?";
    ...
    db.run(deleteQuery, deleteParams, function (err) {
      if (err) {
        console.error(
          "Error deleting instruction step from recipe:",
          err.message
        );
        res.status(500).json({ error: "Internal server error" });
        return;
      ...
      if (this.changes === 0) {
        ...
        res.json({
          message: "Instruction step deleted successfully from the recipe",
        });
      }
    });
  }
...
app.delete("/recipes/:recipeId/delete", (req, res) => {
  ...
  db.run("BEGIN TRANSACTION", function (err) {
    ...
    db.run(
      "DELETE FROM InstructionStep WHERE recipe_id = ?",
      [recipeId],
      function (err) {
        if (err) {
        ...
        db.run(
          ...
          function (err) {
            ...
            db.run(
              "DELETE FROM RecipeDietaryInfo WHERE recipe_id = ?",
              [recipeId],
              function (err) {
                if (err) {
                  console.error(
                    "Error deleting recipe dietary information:",
                    err.message
                  );
                  db.run("ROLLBACK", () => {}); // Rollback transaction on error
                  res.status(500).json({ error: "Internal server error" });
                ...
                db.run(
                  "DELETE FROM RecipeAllergiesInfo WHERE recipe_id = ?",
                  [recipeId],
                  function (err) {
                    if (err) {
                      console.error(
                        "Error deleting recipe allergies information:",
                        err.message
                      );
                      db.run("ROLLBACK", () => {}); // Rollback transaction on error
                      res.status(500).json({ error: "Internal server error" });
                    ...
                    db.run(
                      ...
                      function (err) {
                        ...
                        db.run("COMMIT", () => {
                          res.json({
                            message:
                              "Recipe and related information deleted successfully",
                          });
                        });
                      }
                    );
...
app.delete("/cuisines/:cuisineId/delete", (req, res) => {
  ...
  db.get(
    "SELECT cuisine_id FROM Cuisines WHERE name = ?",
    ["International"],
    (err, row) => {
      if (err) {
        console.error(
          'Error checking for "International" cuisine:',
          err.message
        );
        res.status(500).json({ error: "Internal server error" });
        return;
      ...
      if (!internationalCuisineId) {
        db.run(
          ...
          function (err) {
            if (err) {
              console.error(
                'Error adding "International" cuisine:',
                err.message
              );
              res.status(500).json({ error: "Internal server error" });
              return;
      ...
      if (internationalCuisineId) {
        db.run(
          "UPDATE Recipes SET cuisine_id = ? WHERE cuisine_id = ?",
          [internationalCuisineId, cuisineId],
          function (err) {
            if (err) {
            ...
            db.run(
              ...
              function (err) {
                ...
                res.json({
                  message:
                    "Cuisine deleted and related recipes updated successfully",
                });
              }
            );
          }
        ...
        res.json({
          message:
            'Cuisine deleted successfully, "International" cuisine not found',
        });
      }
    }
  );
